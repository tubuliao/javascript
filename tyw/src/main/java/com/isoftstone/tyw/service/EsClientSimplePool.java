 ...
 *
 */
public class EsClientSimplePool {
	private final static int INIT_NUMBER_FOR_POOL = 10; // 对象池初始的大小
	private final static int MAX_NUMBER_FOR_POOL = 50; // 对象池最大的大小
	private Vector<PooledObject> objects = null; // 存放对象池中对象的向量( PooledObject类型)

	public EsClientSimplePool() {
	}
	...
	public synchronized void createPool() {
		...
		for (int i = 0; i < INIT_NUMBER_FOR_POOL; i++) {
			if ((objects.size() == 0)
					&& this.objects.size() < MAX_NUMBER_FOR_POOL) {
				Client client = new TransportClient().addTransportAddress(new InetSocketTransportAddress("192.168.0.198",9300));
				objects.addElement(new PooledObject(client));
			}
		}
	}
	...
	public synchronized void createObjects() {
		for (int i = 0; i < INIT_NUMBER_FOR_POOL; i++) {
			if (this.objects.size() < MAX_NUMBER_FOR_POOL) {
				Client client = new TransportClient().addTransportAddress(new InetSocketTransportAddress("192.168.0.198",9300));
				objects.addElement(new PooledObject(client));
			}
		}
