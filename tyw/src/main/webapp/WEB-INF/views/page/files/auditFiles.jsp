...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	...
	function saveAndAuditPass(){
		...
		if(state == 0 || state == 2) {	// 未审批 驳回
			$.messager.confirm('确认', '你确认保存并审批通过该文件吗?', function(r){
				if (r){
					...
					saveForm();
					// 审批通过
					$.post('${pageContext.request.contextPath}/files/approve/'+id,{status:'1'},function(result){
						if (result.success){
							window.close();
						} else {
		...
		if(state == 1) {	// 已审批
			...
			var baseSource = '${file.source}';
			$.messager.confirm('提示：', '该记录已经通过审批，是否进行二次审批？', function(r) {
				if(r) {
					// 保存	
					saveForm();
					// 审批通过
					$.post('${pageContext.request.contextPath}/files/approve/'+id,{status:'3', weightType: weightType, weightVal: weightVal, baseSource: baseSource},function(result){
						if (result.success){
							window.close();
						} else {
							$.messager.show({	// 显示错误信息
	...
	function auditNoPass(){
		...
		$.messager.prompt('驳回原因', '请填写驳回原因:<br><font color="red">注意：必须填写驳回原因！</font>', function(r){
			if (r){
					$.post('${pageContext.request.contextPath}/files/approve/'+id,{status:'2',auditInfo:r},function(result){
					if (result.success){
						window.close();
					} else {
						$.messager.show({	// 显示错误信息
...
<body>
		<form id="fm" method="post" novalidate>
			...
			<!-- 设置权重 -->
			<c:if test="${file.state == 1 || file.state == 3}">
				<br />
				<div class="fitem">
					...
					权重值：<input type="text" name="weighingVal" id="weighingVal" value="${file.weighing }"style="width:40px;" onblur="checkWeighingVal(value);" />
				</div>
				<br />
			</c:if>
    ...
    <input value="" id="parentId" type="hidden"/> 
	 <div id="" style="padding:20px;">
	 	<c:if test="${file.state == 0 || file.state == 2 }"> 
