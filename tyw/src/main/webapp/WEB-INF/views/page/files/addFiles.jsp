    ...
    /*
	        $("#uploadify").uploadify({
				...
				//alert(data);
		        	...
		        	$('#fileContainer').append('<img src="${pageContext.request.contextPath}/images/attach.gif">').append('The file ' + file.name + ' was successfully uploaded with a response of ' + response ); //.append(data);
					//resultsX += data + "|";
					resultsX += file.name.substring(0, file.name.indexOf('.')) + data + "|";
    				$('#results').val(resultsX);
			...
			*/
			...
			$("#uploadify").uploadify({
				...
				//alert(data);
    				//$('#fileContainer').empty(); //清空默认列表
		        	$('#fileContainer').append('<img src="${pageContext.request.contextPath}/images/attach.gif">').append(data);  //.append('The file ' + file.name + ' was successfully uploaded with a response of ' + response ); 
					resultsX += ';' + data;
					//resultsX += file.name.substring(0, file.name.indexOf('.')) + data + "|";
    				$('#url').val(resultsX);
					//alert("results -- " + results) ;
...
<body>
	...
	<div id="dlg-buttons">
		<a href="#" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveFiles()">保存</a>
