...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    ...
    <script type="text/javascript">
		...
		function deletePic(picUrl, id, type) {
			//alert(picUrl + '     ' + id) ;
	
			var url = picUrl.replace(new RegExp('/', 'gm'), '&')//.replace(new RegExp('\\.', 'gm'), '%').replace(new RegExp(':', 'gm'), '%') ;  // url格式转换，get方法传到后台
			//var url = '&&&&'
...
<body>
		<form id="fm" method="post" novalidate>
			...
			<div class="fitem">
				<label>标题:</label>
				<input name="title" class="easyui-validatebox" required="true" value="${form.title }_副本" style="width:400px" />
			</div>
			<div class="fitem">
				<label>简标题:</label>
			...
			<div class="ftitle">标签信息&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">(请修改地区标签)</font></div>
			<div class="fitem">
			...
			<div class="fitem">
				...
				<div id="fileContainer">
					<span style="margin-left:5px;"><img src="${pageContext.request.contextPath}/images/attach.gif" /></span>
				</div> 
			...
			<div class="fitem">
				...
				<!--
					<c:forEach items="${hiPicUrlSet}" var="hiPicUrl">
						<span style="margin-left:5px;"><img src="${pageContext.request.contextPath}/images/attach.gif" /><a href='${hiPicUrl}' alt='查看附件' onclick="window.open(this.href, '_blank', '');return false">${hiPicUrl}</a>&nbsp;&nbsp; <input type="button" value='删除' id='${i}' onClick='deletePic("${hiPicUrl}", "${form.id}", "hi");'></span></br>
					</c:forEach>
				-->
				</div>
			...
			<!--
					<c:forEach items="${lowPicUrlSet}" var="lowPicUrl">
						<span style="margin-left:5px;"><img src="${pageContext.request.contextPath}/images/attach.gif" /><a href='${lowPicUrl}' onclick="window.open(this.href, '_blank', '');return false">${lowPicUrl}</a>&nbsp;&nbsp; <input type="button" value='删除' onClick='deletePic("${lowPicUrl}", "${form.id}", "low")'></span></br>
					</c:forEach>
			-->
				</div>
	...
	<div id="dlg-buttons">
		<a href="#" class="easyui-linkbutton" iconCls="icon-ok" onclick="saveForm()">另存为……</a>
