    ...
    <script type="text/javascript">
    	...
    	function saveAndAuditPass(){
    		...
    		if(state == 0 || state == 2) {	// 未审批 驳回
    			$.messager.confirm('确认', '你确认保存并审批通过该图片吗?', function(r){
        			if (r){
        				// 保存	
        				saveImage();
        				
        				// 审批通过
        				$.post('${pageContext.request.contextPath}/image/approve/'+id,{status:'1'},function(result){
        					if (result.success){
        						window.close();
        					} else {
    		...
    		if(state == 1) {	// 已审批
    			...
    			$.messager.confirm('确认', '你确认保存并审批通过该图片吗?', function(r){
        			if (r){
        				...
        				// 审批通过
        				$.post('${pageContext.request.contextPath}/image/approve/'+id,{status:'3', weightType: weightType, weightVal: weightVal, baseSource: baseSource},function(result){
        					if (result.success){
        						window.close();
        					} else {
        						$.messager.show({	// 显示错误信息
    	...
    	function reject(){
    		...
    		$.messager.prompt('驳回原因', '请填写驳回原因:<br><font color="red">注意：必须填写驳回原因！</font>', function(r){
    			if (r){
    					$.post('${pageContext.request.contextPath}/image/approve/'+id,{status:'2',auditInfo:r},function(result){
    					if (result.success){
    						window.close();
    					} else {
    						$.messager.show({	// 显示错误信息
...
<body>
		<form id="fm" method="post" novalidate>
			...
			<!-- 设置权重 -->
			<c:if test="${image.state == 1 || image.state == 3}">
				<br />
				<div class="fitem">
					...
					权重值：<input type="text" name="weighingVal" id="weighingVal" value="${image.weighing }" style="width:40px;" onblur="checkWeighingVal(value);" />
				</div>
				<br />
			</c:if>
    ...
    <div id="" style="padding:20px;">
		<c:if test="${image.state == 0 || image.state == 2 }">
