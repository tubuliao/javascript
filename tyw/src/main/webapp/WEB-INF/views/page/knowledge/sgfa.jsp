...
<script src="${pageContext.request.contextPath}/jquery-ui-1.10.2/ui/jquery-ui.js"></script>
		...
		<script type="text/javascript">
			...
			function beforeExpand(treeId, treeNode) {
				...
				if(treeNode.code>=40302050000000000 && 40302059999999999>treeNode.code){
					disabledFbfx(false);
				}else{
					disabledFbfx(true);
				}
			...
			function disabledFbfx(isDisplay){
				if(isDisplay ==true){
					$("a[name='a_fbfx']").each(function(){
						$(this).attr("class",$(this).attr("temp"));
					});
					$("a[name='a_fbfxs']").each(function(){
						$(this).attr("class",$(this).attr("temp"));
					}); 
				}else{
					...
					$("a[name='a_fbfx']").each(function(){
						...
						$(this).attr("temp",$(this).attr("class"));
		...
		<div class=" new_box">
            ...
            <div class="new_position">
				...
				<div class="new_left" id="left_banner" style="overflow: hidden">
					...
					<ul class="now" id="fbfx">
						<c:forEach items="${Source}" var="source" varStatus="status">
                    	<!-- <li onclick="getSourceList('核心条文')"> -->
                    	<li>
                    		<c:if test="${source.count!=0}">
                    			<span class='left'><a name="a_fbfxs" title="${source.name}" href="javascript:void(0)" class="a_clicks" data="${source.code}"> ${source.name}&nbsp;&nbsp;<span style="color:green">(${source.count})</span></a></span>
				...
				<div class="new_right">
                    ...
                    <div class="new_search_title">查询条件</div>
						<form>
							<table width="100%" border="0" cellpadding="0" cellspacing="0"
								...
								<tr>
									...
									<!-- td width="9%" height="30" align="right"><div>地域</div></td>
									<td width="24%" height="30"><input
										class="xmm_input_s width90" type="text" name="province"
										id="province" value="" />
										<div id="chooseArea" class="fcontrol" title="省份"
											style="width: 250px; height: 100px; display: none"></div></td-->
						...
						<button onclick="sub()" class="new_chaxun">查询</button>
                        </div>
					</div>
