function removePpt(){
	...
	if (row){
		$.messager.confirm('提示','您确定要删吗?',function(r){
			if (r){
				$.post('/ppt/delete/'+row.id,{id:row.id},function(result){
					if (result.success){
...
function okPpt(){
	...
	if(row){
		...
		$.messager.confirm('确认', '你确认审批通过该文件吗?', function(r){
			if (r){
				$.post('ppt/approve/'+row.id,{status:'1'},function(result){
							if (result.success){
								$('#grid').datagrid('reload');	// 重新加载数据
							} else {
								$.messager.show({	// 显示错误信息
...
function breakPpt(){
	...
	if(row){
		...
		$.messager.prompt('驳回原因', '请填写驳回原因:<br><font color="red">注意：必须填写驳回原因！</font>', function(r){
			if (r){
					$.post('ppt/approve/'+row.id,{status:'2',auditInfo:r},function(result){
					if (result.success){
						$('#grid').datagrid('reload');	// 重新加载数据
					} else {
						$.messager.show({	// 显示错误信息
...
function showPpt02(){
	...
	var row = $('#grid').datagrid('getSelected');
	if (row){
		$.get('/ppt/previewPpt', { id: row.id}, function(result) {
			if(result.success) {
				var pptHtml='';
				var f = result.ppt;
				var fPage = result.pptPageList;
				for(var j=0;j<fPage.length;j++){
					if(fPage[j] != null && fPage[j] != "") {
						pptHtml += "<img src='" + fPage[j].url + "' onload=\"resizeImg(this);\" /> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" ;
					}
				}
				$parent("#pptContent").html(pptHtml);
				$parent("#pptTitle").html("标题："+f.title);
				$parent("#pptSource").html("来源："+ (f.source==null?"(无)":f.source)) ;
				$parent("#pptCreateDate").html("创建时间："+f.createDate);
				$parent("#pptInsertName").html(f.insertName==null?"录入人：(无)":"录入人："+f.insertName);
				$parent("#pptId").val(f.id);
				var tagNames=[];
				for(var i=0;i<f.tags.length;i++){
					tagNames.push(f.tags[i].name);
				}
				$parent("#pptTags").html("标签："+tagNames.join(","))
				pptWin.window('open');
			} else {
...
function showPpt(){
	...
	var row = $('#grid').datagrid('getSelected');
	if (row){
		window.open('/detail/6/' + row.id, "_blank", "toolbar=no, location=yes, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, copyhistory=yes, width=1024, height=800");
	}else {
        Msgshow('请先选择要查看的记录。');
    }
}
...
$(function () {
	...
	grid=$('#grid').datagrid({
		        ...
		        columns: [[
							...
							{ field: 'urls', title: '同步状态', align: 'center', width: 25, formatter:function(value, row, index) {if(value != null && value != "") {return '<font color="green">已同步</font>' ;} else {return '<font color="red">未同步</font>' ;}}	},
			...
			pptWin=$parent('#ppt-window').window({
		        closed: true,
		        modal: true,
...
function editPpt() {
    var row = grid.datagrid('getSelected');
    if(row){
//    	location.href='/ppt/edit/'+row.id;
    	window.open('/ppt/edit/'+row.id, "_blank", "toolbar=no, location=yes, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, copyhistory=yes, width=1024, height=800");
    	// 被编辑数据行标题着色
    	showSelectedRow(row);
    }else{
...
function synPptAttachment() {
	...
	$.get('/ppt/synPptAttachment', function(result) {
		if(result.success) {
			$.messager.progress('close') ;
...
function okOrBreak() {
	var row = grid.datagrid('getSelected');
    if(row){
		window.open('/ppt/audit/' + row.id, "_blank", "toolbar=no, location=yes, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, copyhistory=yes, width=1024, height=800");
		// 被编辑数据行标题着色
    	showSelectedRow(row);
    }else{
    	 Msgslide('请选择一条记录进行操作!'); //$.messager.alert('提示', '请选择一条记录进行操作!', 'info');
