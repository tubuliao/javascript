...
//$(document).ready(function(){
	 ...
	 */ 
		...
		function getTagListByParentId(code){
			var result = '';
			htmlobj=$.get("/doTableTagListajax",{code:code},
		...
		$(function(){
			...
			$(".a_click").live("click",function(){
				...
				if(parent.attr('ajax_status')=="1"){
					...
					$.get("/doTableTagListajax",{code:now_id},function(data){
						if(data.length >0){
							$("#fbfxtreetitle").text(show_text);
							for(var i=0;i<data.length;i++){
								html_string = html_string + "<li onclick='getSourceList()'><span class='left'><a href='javascript:void(0)' class='a_clicks' data='"+data[i].code+"'>"+data[i].name+"</a></span>";
								if(data[i].leaf==0){
									html_string = html_string +"<a href='javascript:void(0)' style='display:inline-block;float:right' class='a_click' data='"+data[i].code+"' text='"+data[i].name+"'><img  class='right ' src='/images/new/new_17.gif' title='点击进入下一级' /></a>";
								}
			...
			$("a.path").live("click",function(){
				...
				$("#fbfxtreetitle").text($(this).text().replace(">",""));
			...
			$(".back_link").click(function(){
				...
				// alert($(".path").eq(data_index).text());
				//$("#lastpath").val($(".path").text().replace(">",""));
				if (max>1){
					data_id = $(".path").eq(data_index-1).attr("data");
					$("#fbfxtreetitle").text($(".path").eq(data_index-1).text());
					$("#bak_ul ul").each(function(index){
						var temp_id = $(this).attr('data');
						if (data_id == temp_id) {
							$(this).clone(true).attr('ajax_status',"0").appendTo("#left_banner").addClass('next');
